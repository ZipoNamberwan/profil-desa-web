                ...
                <div class="col-lg-6 col-7">
                    <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
                        <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
                            ...
                            <li class="breadcrumb-item"><a href="/characteristics">Karakteristik</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Ubah Karakteristik</li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <div class="card-wrapper">
                ...
                <div class="card">
                    ...
                    <div class="card-header">
                        <h3 class="mb-0">Ubah Karakteristik</h3>
                    </div>
                    <!-- Card body -->
                    <div class="card-body">
                        <form id="formupdate" autocomplete="off" method="post" action="/characteristics/{{$characteristic->id}}" class="needs-validation" enctype="multipart/form-data" novalidate>
                            @method('PUT')
                            @csrf
                            <div class="row">
                                <div class="col-md-4 mb-3">
                                    <label class="form-control-label" for="name">Nama Karakteristik<span class="text-danger">*</span></label>
                                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" id="validationCustom03" value="{{ @old('name', $characteristic->name) }}">
                                    @error('name')
                                            ...
                                            <div class="table-responsive py-2 scrollable">
                                                <table class="table" width="100%" id="row-table">
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            <td class="px-1">1</td>
                                                            <td class="px-1 pr-5"><input id="rowidfirst" type="hidden" name="rowid[]" value="{{ old('rowid.0', $characteristic->items[0]->id) }}">
                                                                <input class="form-control" type="text" id="itemfirst" name="item[]" value="{{ old('item.0', $characteristic->items[0]->name) }}">
                                                                @error('item.0')
                                                        ...
                                                        @for ($i = 1; $i < count(old('item')); $i++) <tr>
                                                            ...
                                                            @endfor
                                                            @else
                                                            @for ($i = 1; $i < count($characteristic->items); $i++)
                                                                <tr>
                                                                    <td class="px-1">{{$i+1}}</td>
                                                                    <td class="px-1 pr-5"><input type="hidden" name="rowid[]" value="{{ $characteristic->items[$i]->id }}"><input class="form-control d-inline mr-2" type="text" id="itemfirst" name="item[]" value="{{ $characteristic->items[$i]->name}}"><button id="btnName{{ $i }}" onclick="removerow('btnName{{ $i }}')" class="btn btn-icon btn-sm btn-outline-danger d-inline" type="button"><span class="btn-inner--icon"><i class="fas fa-trash-alt"></i></span></button></td>
                                                                </tr>
                                                                @endfor
                                                                @endif
                                                                <tr>
...
<script>
    var rowcount = @if(old('item', $characteristic->items)) {{count(old('item', $characteristic->items))}}@else 1 @endif;
    ...
    function appendremovedrow(id) {
        var form = document.getElementById('formupdate');
        var hidden = document.createElement("input");
        hidden.setAttribute("type", "hidden");
        hidden.setAttribute("name", "removedrow[]");
